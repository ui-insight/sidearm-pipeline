"""Reusable boxscore ingestion logic.

This module provides the core ingest function used by both the HTTP endpoint
(``api/v1/games.py``) and the background scheduler (``services/scheduler.py``).
"""
...
async def ingest_boxscore_url(
    url: str,
    db: AsyncSession,
    *,
    trigger_type: str = "manual",
) -> Game:
    """Fetch a Sidearm boxscore URL, parse it, and persist the result.

    Creates a new canonical event record on first ingest; refreshes an
    existing one on subsequent ingests (idempotent by ``canonical_uid``).
    Always records an ``IngestRun`` for operational history.

    Args:
        url: Full Sidearm boxscore page URL.
        db: Active async database session.
        trigger_type: Origin label stored on the ``IngestRun`` record.
            Use ``"manual"`` for UI-triggered ingests, ``"scheduled"`` for
            background scheduler runs, and ``"manual_rerun"`` for re-runs.

    Returns:
        The created or refreshed ``Game`` ORM instance (loaded with all
        related collections).

    Raises:
        httpx.HTTPError: When the Sidearm fetch fails after all retries.
    """
...
def _finish_failed_ingest_run(
    ...
    ingest_run.run_metadata = {
        ...
        "retry_exhausted": (
            ingest_run.retryable
            and ingest_run.attempt_count >= ingest_run.max_attempts
        ),
    }


# ---------------------------------------------------------------------------
# Game load / build / refresh
# ---------------------------------------------------------------------------


async def _load_existing_game(
    db: AsyncSession, parsed: ParsedBoxscore
) -> Game | None:
