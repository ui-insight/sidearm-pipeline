...
| Notability policy | Versioned WBB rubric seeded from reviewed repository configuration | Assigns metric importance, career thresholds, scope weights, and a top-N boundary without changing metric semantics | `notability_policies`, `notability_policy_metrics` | deterministic achievement detector; later SID tuning workflow | Existing versions are immutable historical inputs; tuning creates a later version |
| Achievement Suggestions | Final, non-exhibition Idaho player-game facts plus earlier eligible warehouse game facts | SQLAlchemy-authored queries detect and score verified achievements; an optional model call can reorder candidates and phrase only supplied facts after strict output validation | `achievement_suggestions` | SID review and later Verdict workflow | Stores source-snapshot and Coverage Window context plus model, prompt-version, output-hash, and ranking-time provenance; no model generates or changes facts or numbers; invalid output is not persisted |
