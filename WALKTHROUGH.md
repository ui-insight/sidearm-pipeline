...
Ask plain-English questions about the games database. The agent generates a
`SELECT`-only SQL query, executes it, and returns a natural language answer.
...
**Constraints:** The agent only issues `SELECT` statements. Any non-`SELECT`
SQL generated by the model is rejected before execution.
...
The `steps` array records each phase of the agent:

| Step name | recap-writer | nl-query |
|-----------|-------------|----------|
| `serialize_input` / `introspect_schema` | order 0 | order 0 |
| `call_model` | order 1 | order 1 |
| `parse_output` / `execute_sql` | order 2 | order 2 |
| — | — | `format_response` (order 3) |
...
All 65 tests should pass. The test suite uses SQLite in-memory by default — no
