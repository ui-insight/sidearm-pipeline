...
test("season desk assembles evidence and exports it", async ({ page }) => {
  ...
  await page.route("**/api/v1/semantic-queries/execute", async (route) => {
    const body = route.request().postDataJSON() as { query_id: string };
  ...
  await expect(page.getByText("Washington State")).toBeVisible();
  ...
  expect(download.suggestedFilename()).toBe("wbb-2025-26-points-all.csv");
});
